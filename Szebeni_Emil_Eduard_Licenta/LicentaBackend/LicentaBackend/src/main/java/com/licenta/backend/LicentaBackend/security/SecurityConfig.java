...
import com.licenta.backend.LicentaBackend.security.jwt.JwtAuthorizationFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    @Autowired
    private CustomDoctorSpitalDetailsService customDoctorSpitalDetailsService;

    @Autowired
    private CustomAdminDetailsService customAdminDetailsService;

    @Autowired
    private CustomFarmacistDetailsService customFarmacistDetailsService;
...
//    }
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.userDetailsService(customDoctorSpitalDetailsService).passwordEncoder(passwordEncoder());
        auth.userDetailsService(customAdminDetailsService).passwordEncoder(passwordEncoder());
        auth.userDetailsService(customFarmacistDetailsService).passwordEncoder(passwordEncoder());
    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .antMatchers(HttpMethod.PUT,"/api/trimitere/**").hasRole(Role.DOCTORFAM.name())// sau .hasRole(Role.ADMIN.name(), doar cei cu rol de Admin pot sa acceseze endpointul
                .antMatchers(HttpMethod.GET,"/api/trimitere/**").permitAll()
