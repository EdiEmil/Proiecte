...
function HomePage() {
  ...
  const purchase = (product) => {
    ...
    PurchaseService.savePurchase(purchase).then(() => {
      ...
      setErrorMessage('Unexpected error for purchase');
      console.log(err);
    });
  }
    ...
    <div className='container p-3'>
      ...
      <div className='d-flex flex-wrap'>
        {productList.map((item, ind) =>
          <div key={item.id} className='card m-3 custom-home-card'>
            ...
            <FontAwesomeIcon icon={faCartPlus} className="ms-auto me-auto custom-product-icon" /* imaginea cu caruciorul*/ />
